      ...
      <div className="flex flex-col sm:flex-row sm:items-stretch bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl shadow-2xl">
        ...
        <div className="flex items-stretch border-b border-white/10 sm:contents">
          <DatePickerCell
            ...
            onChange={setDepart} iconColor="text-sky-300" accent="sky"
            className="relative flex-1 min-w-0 border-r border-white/15"
          />
          {tripType === 'return' && (
            <DatePickerCell
              label="Return" value={ret} min={depart || today}
              onChange={setRet} iconColor="text-sky-300" accent="sky"
              className="relative flex-1 min-w-0 border-r border-white/15"
            />
          )}
        </div>
        ...
        <div className="flex items-stretch sm:contents">
          <div className="relative flex-1 sm:flex-none" ref={passRef}>
            <button onClick={() => { setPassOpen(v => !v); setClassOpen(false) }}
              ...
              <Users className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-sky-300 shrink-0" />
              <div>
          ...
          <button onClick={go}
            className="px-3 sm:px-5 bg-gradient-to-r from-sky-500 to-blue-600 text-white font-bold text-xs sm:text-sm hover:opacity-90 transition-all flex items-center justify-center gap-1.5 sm:gap-2 flex-1 sm:flex-none rounded-b-2xl sm:rounded-bl-none sm:rounded-r-2xl">
